...
- 🆕**하우스 스타일 프리셋** — `@iyulab/enterprise/styles/preset.css`
  ...
  **계약 셋** (`tests/preset-contract.test.ts` 가 강제한다):
  ...
  ⑶ 🔴**소비자가 항상 이긴다** — 선택자를 `:where()` 로 감싸 **특이도 0** 으로 선언한다.
     소비앱의 평범한 `:root { … }` 한 줄이 **로드 순서와 무관하게** 이긴다.
     ★종전에 소비앱들이 `:root:not([theme="dark"])` 같은 **특이도 방어**를 쓰고 있었는데,
     그것은 업스트림이 `:root` 로 선언해 *"뒤에 온 쪽이 이기는"* 상황을 만들었기 때문이다.
     특이도 0 으로 선언하면 그 방어가 통째로 필요 없어진다.
